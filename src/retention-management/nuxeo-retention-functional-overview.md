...
- New menus are available in the left side menu:</br>
    ICON: A retention menu including the following sub-menus:
        ...
        ICON: A retention event menu that lets you create a retention event.
...
#### {{> anchor 'retention-events'}}Period starting point
 ...
 - **Immediate**: The retention period will start once you applied the retention rule to the document.

 Example: If I apply a retention rule to a document the 1st January of 2020 with the option **Immediate** and a period of 2 years, then the document will expire the 1st January of 2022.
 ...
 - **Based on an event**: The retention period will start once the defined event will occur (after the retention rule has been applied to the document).
    ...
    - **Event type**: Select the type of event that will trigger the retention period. The list of available event types is defined using a dedicated vocabulary **TBD**
    Examples: Contract terminated, Employee left the company, Repayment of loan,...

    - **Event field value**: Fill the value related to the event
    Examples: Contract number, employee identifier, loan reference,...

 Example: If I apply a retention rule to a document the 1st January of 2020 with the option **Based on an event** "contract terminated", a period of 2 years, and the contract is closed the 10th August of 2020, then the document will expire the 1Oth August of 2022.
 ...
 - **Based on a metadata**: The retention period will start based on a defined metadata, once you applied the retention rule to the document.
  ...
  - **Metadata**: The path of the metadata to use. All metadata of all schemas are available (default and local ones).
  Examples: dc:title, contract:number, contract:employeeid,...

 Example: If I apply a retention rule to a document the 1st January of 2020 with the option **Based on a metadata** "publication date", a period of 2 years, and the "publication date" is 15th of September 2019, then the document will expire the 15th September of 2021.
...
Be aware that it is an **irreversible action**, even for an administrator, meaning:
...
### Extend retention of a document

When a document is under retention and/or a legal hold, you can lengthen the retention of the document.

Be aware that this action is an **irreversible action**: there is no way to shorten a retention period, even for an administrator.
...
2. Select the new expiration date (end of retention period) and click on **Extend retention**.</br>
    A top bar is displayed next to your document to notify users of the new expiration date.
    SCREENSHOT
...
2. Fill the description and click on **Apply Legal Hold**.</br>
    A top bar is displayed next to your document to notify users that the document is under a Legal Hold.
    SCREENSHOT
...
2. Click on **Remove Legal Hold**.</br>
    A top bar is displayed next to your document to notify users that the document is not under a Legal Hold anymore.
    SCREENSHOT
...
3. Click on the **Create** button. </br>
    ...
    - **Event type**: Select the type of event you want to create among a list of available types
    Examples: Contract terminated, Employee left the company, Repayment of loan,...
