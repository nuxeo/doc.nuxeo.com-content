...
This is the main document type. It holds the holiday request data : beginning date, end date, managers, number of each vacation day type... Holiday requests are associated with lifecycle HRHolidayRequestLifecycle that holds request lifecycle states, from draft to accepted trough operational and then HR validation.

A "request vacation period" button is created in Human Resource folder by the User Action "NewHolidayRequestAction" that triggers the Automation Chain "CreateHolidayRequestChain". This chain first navigates to the user holiday folder and then shows the Holiday Creation page.
...
The solution is to use a vocabulary and the function `@{Fn.getVocabularyLabel("Vocabulary","id"}`. The vocabulary is used to store the operational manager of each user, the id will be the user, and the label will be operational manager.
...
Here is an example where jack is the operational manager of john:
...
Then document lifecycle is set to "op_manager_approval", several properties of the document are updated (request creation date...) and Read & Edit permission is granted to the operational manager so that he can make modifications if needed.

*   Operational manager validation

When the operational manager validates his task, it launches "OMValidatedChain". This chain set the document lifecycle state to "hr_manager_processing", creates a task to HR manager and send an email to both HR manager and the request creator.
...
During the different steps of the workflow, people have different needs of accessing the requests, those needs change with lifecycle states. For example, operational manager does not need the Edit permission when the request is in draft state.
...
When going the human resource folder, each user has access to different views depending on his role. For example, an operational manager can see all pending request of his team.
This view are defined in the search and listing section of Nuxeo studio and are displayed in additional tabs of the human resources folder depending once again on user role.

![]({{file name='screenshot_2011-08-30_12.17.12.png'}} ?border=true)

*   The user menu human resource link:

The studio project adds an entry directly in the user menu to navigate to human resource folder thanks to the user action "GoToHR". The bound automation chain is "GoToHRSpaceChain". This chain not only navigates to the user's humane resource folder and but also checks that it exists and if not, launches another automation chain to create the human resource folder.
