...
confluence:
    ...
    canonical_source: 'https://doc.nuxeo.com/display/NXDOC710/How+to+Define+a+New+Content+View'
    page_id: '28475839'
    shortlink: v4GyAQ
    shortlink_source: 'https://doc.nuxeo.com/x/v4GyAQ'
    source_link: /display/NXDOC710/How+to+Define+a+New+Content+View
history:
    - 
        author: Manon Lumeau
        date: '2016-04-27 13:28'
        message: ''
        version: '60'
    - 
        author: Manon Lumeau
...
## <a name="set-query-filter"></a>"Set Your Query Filter
...
*   `#{empty currentDocument.collection.documentIds ? '%' : currentDocument.collection.documentIds}` to get the content of a collection. In this case the Query filter should be something like ... `AND [ecm:uuid](http://ecmuuid) IN (?)`

{{/callout}}

More information on NXQL and examples are available on the [NXQL dedicated page]({{page page='nxql'}}).

## <a name="define-filter-form"></a>"Define the Filter Form
...
## <a name="configure-result-layout"></a>"Configure the Result Layout
