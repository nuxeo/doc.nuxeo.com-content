...
#### OAuth 2 Authorization and Bearer Token Authentication

Since Nuxeo Platform 9.2, you can use OAuth 2 authorization through the JS client.

For more information on OAuth 2 server side, see [Using OAuth 2](https://doc.nuxeo.com/nxdoc/using-oauth2/).
...
A typical OAuth 2 flow with the Nuxeo Platform would be:
...
Returns the OAuth 2 authorization URL for the given `baseURL` and `clientId`.
...
Fetches an OAuth 2 access token for the given `baseURL`, `clientId` and `code`.
...
Fetches an OAuth 2 access token for the given `baseURL`, `clientId` and `jwtToken`.
