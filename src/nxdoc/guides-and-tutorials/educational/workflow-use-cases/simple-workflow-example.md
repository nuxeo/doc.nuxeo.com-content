...
confluence:
    ...
    canonical_source: 'https://doc.nuxeo.com/display/NXDOC60/Simple+Workflow+Example'
    page_id: '22380580'
    shortlink: JIBVAQ
...
*   Give Jack read and write access on the domain.
...
3.  Give the workflow an id ("`SalesProposalWorkflow`"), a label and a description.
    ![]({{file name='Simple_Workflow_000.png' space='nxdoc' page='simple-workflow-example'}} ?w=400,h=303,border=true)
...
2.  In the "Current document has one of the types", select **File**.
    ![](https://www.lucidchart.com/publicSegments/view/54edf340-4668-4d4f-9aaf-048a0a004dcf/image.png ?w=400,h=378,border=true)
...
![](https://www.lucidchart.com/publicSegments/view/54edf669-f17c-4acf-9dc7-65050a004b79/image.png ?w=400,h=333,border=true)
...
1.  Hover on the&nbsp;**Accept/Reject** node you just dropped and click on the **Edit** icon&nbsp;![]({{file name='editor_area.gif' space='studio' page='studio-icons-index'}})&nbsp;that appears to open the node properties.
    ![]({{file name='Simple_Workflow_005.png' space='nxdoc' page='simple-workflow-example'}} ?w=150,h=101,border=true)
...
4.  Add the salesman's comment variable: `sales_comment`.
    ![](https://www.lucidchart.com/publicSegments/view/54eeeb8a-ccc0-43bb-a981-01f50a00c10c/image.png ?w=450,h=138,border=true)
5.  Click on **Save**.
...
2.  Link the Start node to the Technical validation node, by pulling an arrow between the unique transition point of the Start node to the input point of the Technical validation node.
    ...
    ![]({{file name='Simple_Workflow_012.png' space='nxdoc' page='simple-workflow-example'}} ?w=200,border=true,thumbnail=true)
...
4.  Grant the appropriate rights.
    The internal lawyers may not have the necessary rights to read and/or write a sales proposal document. To avoid an exception to be thrown because of this, you need to grant them the appropriate rights on the document that needs juridical advice temporarily. This is what the &ldquo;Grant permission to task assignees&rdquo; field is used for. In this case, choose the &ldquo;Read & Write&rdquo; permission to let them modify the document. Keep in mind that these rights will only be granted during the operations made on this node, and reverted to their previous state when following a transition to another node.
...
3.  Pull an arrow from there to the juridical advice node&rsquo;s input point.
    ![]({{file name='Simple_Workflow_018.png' space='nxdoc' page='simple-workflow-example'}} ?w=200,border=true,thumbnail=true)
...
4.  Make sure you connect the proposal update node&rsquo;s output point back to the technical validation node&rsquo;s input point too.
    ![]({{file name='Simple_Workflow_024.png' space='nxdoc' page='simple-workflow-example'}} ?w=400,h=333,border=true)
...
3.  In the&nbsp;**General** tab, the "Automation chains" section lets you make use of an existing automation chain, that you may even edit by using the corresponding link. Automation chains can be launched at different moments, depending of your needs. The input operation chain will be played when entering the node, the output chain when the node is being left. In this situation the chain needs to be played from the start, but is not existing yet though.
    ...
    2.  Give an id to your chain : "`SetAsApproved`".
        ![]({{file name='Simple_Workflow_026.png' space='nxdoc' page='simple-workflow-example'}} ?w=350,border=true)
...
2.  Save your work.
    ...
    ![]({{file name='Simple_Workflow_031.png' space='nxdoc' page='simple-workflow-example'}} ?w=450,h=413,border=true)