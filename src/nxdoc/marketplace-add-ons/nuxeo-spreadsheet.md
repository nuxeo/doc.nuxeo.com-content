...
**Note:** When opening the spreadsheet to edit a list of documents it relies on the content view definition to determine the layout and page provider to use. When in standalone mode it relies on a custom `spreadsheet_listing_` layout and on a `spreadsheet_query_` page provider.
