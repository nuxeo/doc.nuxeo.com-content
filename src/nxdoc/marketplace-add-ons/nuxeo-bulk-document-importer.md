...
confluence:
    ...
    canonical_source: 'https://doc.nuxeo.com/display/NXDOC60/Nuxeo+Bulk+Document+Importer'
    page_id: '22380673'
    shortlink: gYBVAQ
...
Since Nuxeo Platform 5.9.3, you can use the&nbsp; [`ecm:primaryType`](http://ecmprimaryType) &nbsp;field to tell the importer to create a specific document type. in the following example, the importer will create a&nbsp;`DesignArt`&nbsp;custom document type for each file in the folder (`da`&nbsp;is the schema's prefix):
...
To use this node type you need to redefine the importer. For that create a&nbsp;`importer-config.xml`&nbsp;in&nbsp;`nxserver/config`, or add the XML in your Studio project (see below).

*   Using the&nbsp;`importer-config.xml`:
...
*   Adding the extension in you Studio project:

    1.  Go to&nbsp;**Advanced Settings**&nbsp;>&nbsp;**XML Extensions**.
    2.  Add a new extension (give the id you want, for example&nbsp;`ConfigureBulkImporter`).
    3.  Add&nbsp;`<require>`&nbsp;and&nbsp;`<extension>`&nbsp;content from the XML above.
        The tags&nbsp;`<?xml version="1.0">`,&nbsp;`<component ...>`&nbsp;and&nbsp;`</component>`&nbsp;must not be added when an XML extension is used in Studio.
    4.  More details on Studio XML Extensions can be find&nbsp;[in the Studio documentation]({{page page='how-to-contribute-to-an-extension'}}).
