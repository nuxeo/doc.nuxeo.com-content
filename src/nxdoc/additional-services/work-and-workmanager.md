...
confluence:
    ...
    canonical_source: 'https://doc.nuxeo.com/display/NXDOC710/Work+and+WorkManager'
    page_id: '28475475'
    shortlink: U4CyAQ
...
The [WorkManager](http://explorer.nuxeo.org/nuxeo/site/distribution/Nuxeo%20Platform-7.10/viewService/org.nuxeo.ecm.core.work.api.WorkManager) service allows you to run code later, asynchronously, in a separate thread and transaction.
...
The details of the configuration can be seen in the [extension point documentation](http://explorer.nuxeo.org/nuxeo/site/distribution/Nuxeo%20Platform-7.10/viewExtensionPoint/org.nuxeo.ecm.core.work.service--queues). Here we'll concentrate on the important points:
...
When using persistent queues (using Redis, see [Redis Configuration]({{page space='admindoc710' page='redis-configuration'}})), then any Nuxeo instances where the queue is configured will pick Work instances from the persistent queue according to the availability of a thread in its thread pool.
