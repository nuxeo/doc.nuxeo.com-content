...
*   Depending on the back-end database and its configuration, different word stemming strategies may be used, which means that `universes` and `universal` (for instance) may or may not be considered the same word. Check your [database configuration]({{page page='database-configuration'}}) for more on this, and the <span class="confluence-link">&nbsp;</span> <span class="confluence-link"><span class="confluence-link">&nbsp;</span> [ <span class="confluence-link">"analyzer" parameter used in the Nuxeo configuration</span> ]({{page page='repository-configuration'}})</span> for your database.
