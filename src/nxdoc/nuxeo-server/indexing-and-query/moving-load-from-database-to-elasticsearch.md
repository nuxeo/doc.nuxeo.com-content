...
For now the select clause support is limited to scalar properties. See the page [Elasticsearch limitations](http://doc.nuxeo.com/display/NXDOC/Elasticsearch+Indexing+Logic#ElasticsearchIndexingLogic-SearchingandLimitations) for more information.
...
If you have set up a [multi repositories configuration](http://doc.nuxeo.com/display/ADMINDOC/Elasticsearch+Setup#ElasticsearchSetup-ConfigurationforMultiRepositories) to query over them just use the `searchOnAllRepositories` option:
