...
This result in an incompatibility when upgrading from Nuxeo LTS 2021 to 2023 on the tables managed by Hibernate.

To upgrade your Database, you will need to remove this Identity column from all concerned tables: `NXP_LOGS`, `NXP_LOGS_EXTINFO` and `NXP_UIDSEQ`.

First, stop any Nuxeo, the upgrade should be done on an idle DB.

Get the primary key constraint name of the concerned table, for example NXP_LOGS, by running this SQL query on each tables:
...
You should have something like `PK__NXP_LOGS__4364C8827603252B` for `NXP_LOGS`, in the following it will be named
 `PK__NXP_LOGS__TO_REPLACE`, you need to replace it.
