...
Take into account the audit related Elasticsearch indexes when backing up / restoring a Nuxeo instance, so make sure you read the [Backing Up and Restoring the Audit Elasticsearch Index](https://doc.nuxeo.com/display/NXDOC/Backup+and+Restore#BackupandRestore-BackingUpandRestoringtheAuditElasticsearchIndex) section.
...
Since Nuxeo 7.3\. See [Disabling Elasticsearch for Audit Logs](https://doc.nuxeo.com/display/ADMINDOC710/Elasticsearch+Setup#ElasticsearchSetup-DisablingElasticsearchforAuditLogs)

Defaults to false on server upgrade, true on new install.

</td></tr><tr><td colspan="1">`audit.elasticsearch.migration`</td><td colspan="1">false</td><td colspan="1">See [Triggering SQL to Elasticsearch Audit Logs Migration](https://doc.nuxeo.com/display/NXDOC/Elasticsearch+Setup#ElasticsearchSetup-TriggeringSQLtoElasticsearchAuditLogsMigration)</td></tr><tr><td colspan="1">`audit.elasticsearch.migration.batchSize`</td><td colspan="1">1000</td><td colspan="1">See [Triggering SQL to Elasticsearch Audit Logs Migration](https://doc.nuxeo.com/display/NXDOC/Elasticsearch+Setup#ElasticsearchSetup-TriggeringSQLtoElasticsearchAuditLogsMigration)</td></tr><tr><td colspan="1">`audit.elasticsearch.indexName`</td><td colspan="1">`${elasticsearch.indexName}-audit`</td><td colspan="1">See [Configuring Nuxeo to Access the Cluster](https://doc.nuxeo.com/display/NXDOC/Elasticsearch+Setup#ElasticsearchSetup-ConfiguringNuxeoToAccessTheClusterConfiguringNuxeotoAccesstheCluster)</td></tr></tbody></table>
...
```
nuxeoctl config --set repository.clustering.id 12345
```
...
Run the script file to [migrate notification subscriptions](https://raw.githubusercontent.com/nuxeo/nuxeo/7.10/nuxeo-distribution/nuxeo-distribution-resources/src/main/resources/bin/upgrade-6.0-7.10/notification-subscriptions-migration.psql.sql) (also available in your Nuxeo distribution):
...
See [Configuring Nuxeo to Access the Cluster](https://doc.nuxeo.com/display/NXDOC/Elasticsearch+Setup#ElasticsearchSetup-ConfiguringNuxeoToAccessTheClusterConfiguringNuxeotoAccesstheCluster)
