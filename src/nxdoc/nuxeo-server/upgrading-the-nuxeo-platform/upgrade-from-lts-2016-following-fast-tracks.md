...
history:
    -
        ...
        date: '2017-04-04 16:23'
        message: 'Initial Version'
        version: '1'

---
{{! multiexcerpt name='8.10-to-9.10-following-FT-upgrade-page'}}
...
** Default Property Value Behavior Changed **
...
** Artifact `org.nuxeo.ecm.platform:nuxeo-platform-el` Renamed **

{{! multiexcerpt name='upgrade-9.1-code.platform.el'}}

Elastic Search artifact has been renamed from `org.nuxeo.ecm.platform:nuxeo-platform-el` to `org.nuxeo.ecm.core:nuxeo-core-el`. See [NXP-21713](https://jira.nuxeo.com/browse/NXP-21713).

{{! /multiexcerpt}}

** New requirement when testing with `RepositoryElasticSearchFeature` **
...
** `Services.CreateUser` and `Services.QueryUsers` Operations **

{{! multiexcerpt name='upgrade-9.1-operation.Server.CreateUser'}}

Operations `Services.CreateUser` and `Services.QueryUsers` are now part of the base platform, renamed into:
...
The existence of the input user name or group name is now checked before adding a permission. See [NXP-21559](https://jira.nuxeo.com/browse/NXP-21559).
...
To fix exports, you can manually edit the zip file, prior to 9.1, by removing `<common:size>` and `<file:filename>` elements from `document.xml`. See [NXP-21025](https://jira.nuxeo.com/browse/NXP-21025).
...
** `FileManager` can now prevent from overwriting existing Document **
...
** `BlobDispatcher` can now dispatch depending document's xpath **
...
** Nuxeo Driver Server - Synchronization Root Behavior **

{{! multiexcerpt name='upgrade-9.1-addons.drive.apis'}}

Synchronization root is resetted with a copied document. In 8.10 and earlier, copying a synchronization root Document, results to create a new synchronization root.
...
- `nuxeo-box-api`: See [NXP-22041](https://jira.nuxeo.com/browse/NXP-22041)
...
* [Release notes for 9.1](http://nuxeo.github.io/releasenotes/9.1/)
