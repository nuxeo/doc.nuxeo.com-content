...
---
The bulk edit screen offers a form that allows to edit several documents at the same time. This is implemented using the [BulkEditService](http://explorer.nuxeo.org/nuxeo/site/distribution/9.10/viewComponent/org.nuxeo.ecm.webapp.bulkedit.BulkEditService) component. You can customize it with Studio by creating a form layout named `bulkEdit`. The "Edit" layout will be used on the bulk edit popup. The default bulk edit layout can be found on [GitHub](https://github.com/nuxeo/nuxeo/blob/release-9.10/nuxeo-dm/nuxeo-platform-webapp-core/src/main/resources/OSGI-INF/layouts-contrib.xml) or in the [Explorer](http://explorer.nuxeo.org/nuxeo/site/distribution/9.10/viewContribution/org.nuxeo.ecm.platform.forms.layouts.webapp--layouts).
...
5.  In the Advanced Configuration part, attach the `layout_bulkedit_template.xhtml` which is in the folder `$NUXEO/nxserver/nuxeo.war/layouts`.
    ...
    See the GitHub documentation about the [layout bulk edit template](https://github.com/nuxeo/nuxeo/blob/release-9.10/nuxeo-jsf/nuxeo-platform-webapp-base/src/main/resources/web/nuxeo.war/layouts/layout_bulkedit_template.xhtml).
...
This versioning policy can be configured through the [versioning](http://explorer.nuxeo.org/nuxeo/site/distribution/9.10/viewExtensionPoint/org.nuxeo.ecm.webapp.bulkedit.BulkEditService--versioning) extension point:
