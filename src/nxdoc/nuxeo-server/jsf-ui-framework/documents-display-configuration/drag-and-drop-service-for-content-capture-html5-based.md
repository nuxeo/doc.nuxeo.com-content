...
* [`Seam.RunOperation`](http://explorer.nuxeo.org/nuxeo/site/distribution/9.10/viewOperation/RunOperation): runs an operation or a chain in the Seam context. For example, if you want to get available actions via the `Actions.GET` operation, but want to leverage the Seam context for actions filters:
...
*   [`Seam.InitContext`](http://explorer.nuxeo.org/nuxeo/site/distribution/9.10/viewOperation/WebUI.InitSeamContext) / [`Seam.DestroyContext`](http://explorer.nuxeo.org/nuxeo/site/distribution/9.10/viewOperation/WebUI.DestroySeamContext): initializes / destroys the Seam context:
