...
Only deployments using the **MongoDB** backend can benefit from this feature. The following conditions must also be met:
 ...
 - repositories must use a blob provider extending `BlobStoreBlobProvider`, such as `S3BlobProvider`, `LocalBlobProvider`, `AzureBlobProvider`, `GoogleStorageBlobProvider` or `GridFSBlobProvider`
