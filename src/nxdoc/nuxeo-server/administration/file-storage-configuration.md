...
---
The files attached to a document in Nuxeo are usually stored separately from the main document database. The way in which they are stored is configurable using the concepts of a  blob provider, Blob Provider and Blob Dispatcher.
...
{{#> panel type='code' heading='Standard LocalBlobProvider Configuration'}}
```xml
...
It is automatically generated by Nuxeo when initializing an empty  blob provider, but you can put it manually and change the configuration:
...
To register a new blob provider, use the [`blobprovider`](http://explorer.nuxeo.com/nuxeo/site/distribution/latest/viewExtensionPoint/org.nuxeo.ecm.core.blob.BlobManager--configuration) extension point and with the class for your  blob provider:
...
Usually, if you don't use the advanced Blob Dispatcher capabilities, you will need one  blob provider per Nuxeo repository. By default Nuxeo uses a  blob provider with the same name as each repository, for instance the "default" repository will use the "default"  blob provider. For a standard Nuxeo instance with a single repository, this is all taken care of for you by the default template.
