...
confluence:
    ...
    canonical_source: 'https://doc.nuxeo.com/display/NXDOC58/Contributing+an+Operation'
    page_id: '17334471'
    shortlink: x4AIAQ
    shortlink_source: 'https://doc.nuxeo.com/x/x4AIAQ'
    source_link: /display/NXDOC58/Contributing+an+Operation
history:
    - 
        author: Solen Guitter
        date: '2015-01-26 16:43'
        message: ''
        version: '29'
    - 
        author: Solen Guitter
...
When registering an automation chain the chain will be checked to find a path from the first operation to the last one to be sure the chain can be executed at runtime. Finding a path means to identify at least one method in each operation that is matching the signature of the next operation. If such a path could not be found an error is thrown (at registration time). For more detail on registering an operation chains see [Contributing an Automation Chain]({{page page='contributing-an-automation-chain'}}).

To register your operation you should create a Nuxeo XML extension to the [`operations` extension point](http://explorer.nuxeo.org/nuxeo/site/distribution/Nuxeo%20Platform-5.8/viewExtensionPoint/org.nuxeo.ecm.core.operation.OperationServiceComponent--operations). Example:
...
*   **script**. Java type: `org.nuxeo.ecm.automation.core.scripting.Expression`
    ...
    For the complete list of scripting objects and functions see [Use of MVEL in Automation Chains]({{page space='nxdoc' page='use-of-mvel-in-automation-chains'}}).
