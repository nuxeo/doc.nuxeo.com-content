...
The configuration starts the [userManager extension point](http://explorer.nuxeo.com/nuxeo/site/distribution/latest/viewExtensionPoint/org.nuxeo.ecm.platform.usermanager.UserService--userManager). For the most frequent use cases (Integrating with LDAP or Active directory), a wizard at first startup time will generate the required XML contributions and deploy them. You just need to select the kind of "directory" you want (SQL, LDAP, multi-directory), and then fill the required information. The wizard will actually generate a contribution to the `userManager` extension point and some contributions to declare users and groups directories, and will copy them in `nxserver/config` folder (ex: default-ldap-users-directory-bundle.xml).
...
for which `somedirectory` has also been contributed. (See [ldap]({{page page='data-lists-and-directories#ldapdirectories'}}) and [sql users]({{page page='data-lists-and-directories#sqldirectories'}}) contributions, as well as [multi-directory]({{page page='data-lists-and-directories#multi-directories'}})).
...
&nbsp;[Contribute an XML extension]({{page space='nxdoc' page='how-to-contribute-to-an-extension'}}) from the following content. Adapt the `authenticationChain` element content with the list of plugins you want to use.
...
OAuth 1 and OAuth 2 are natively supported by the Nuxeo Platform, which means there is no bundle to install and no XML extensions for enabling it. There is an HTTP filter for each, that handles authentication in priority compared to the filter that handles the contributed authentication chain illustrated at the beginning of this section. To configure a key for your client, go to the Nuxeo Platform web application, then browse **Admin Center > Cloud Services > Consumer** tab. You must give a name, a ClientId, and a ClientSecret.
...
If you want to offer ability to get authenticated by a third party provider Open Id/ oAuth2 compatible (like "Login with Google", or "Login with Facebook"), please [follow instructions on this page]({{page page='using-openid-oauth2-in-login-screen'}}).
...
It is possible to integrate the Nuxeo Platform with a Kerberos server installing the [Kerberos Nuxeo Package](https://connect.nuxeo.com/nuxeo/site/marketplace/package/kerberos-authentication) and by adding `KRB5_AUTH` plugin to the authentication chain. Since some additional configuration must be done server-side, [a dedicated documentation page is provided]({{page page='kerberos-authentication'}}).

### CAS2 - CAS

This plugin implements a client for CAS SSO system (Central Authentication System). It can be configured to use a CAS proxy. It has been tested and reported to work with CAS V2\. It is easy to test this plugin by installing the JA-SIG Central Authentication Service Open Source CAS server. To install the CAS2 authentication plugin:
...
3.  Put the CAS2 plugin into the authentication chain.
    {{{multiexcerpt 'authentication_chain_contribution' page='NXDOC710:Authentication and User Management'}}}
...
You can make your Nuxeo Platform instance use services of [DuoWeb](https://www.duosecurity.com/) for benefiting from a two factors authentication (like receiving a code by SMS additionally to putting the login/ password). See the [dedicated addon page]({{page page='nuxeo-duoweb-two-factor-authentication'}}) for usage and installation instructions.
