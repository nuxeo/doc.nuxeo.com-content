  ...
  <script>
    function run(input, params) {
      ...
      var docs = Repository.Query(null, {
        'query': "SELECT * FROM Document WHERE ecm:isTrashed = 0 AND sf:objectid = '" + sfobject.id + "' AND ecm:isCheckedInVersion = 0 AND ecm:mixinType != 'HiddenInNavigation'",
      });
      if (docs.length>0) {
          ...
          if(sfobject.attributes.type === "Account") {
            if(sfobject.ParentId ===  null){
              ...
              var parents = Repository.Query(null, {
                'query': "SELECT * FROM Document WHERE ecm:isTrashed = 0 AND sf:objectId = '" + sfobject.ParentId + "' AND ecm:isCheckedInVersion = 0 AND ecm:mixinType != 'HiddenInNavigation'",
              });
            ...
            if(sfobject.Contract_ID === null) {
              var accounts = Repository.Query(null, {
                'query': "SELECT * FROM Document WHERE ecm:isTrashed = 0 AND sf:objectId = '" + sfobject.AccountId + "' AND ecm:isCheckedInVersion = 0 AND ecm:mixinType !=  'HiddenInNavigation'",
              });
