...
To do:</br>
In your `web.config` on your Sitecore instance, add to your existing Content-Security-Policy the following (if you already have a specific security policy entry, you will need to add the Nuxeo info to that existing policy, just note in the example below the plaement of the various URLs).:
{{/callout}}
